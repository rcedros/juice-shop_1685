...
router.post('/', async (req: Request<Record<string, unknown>, Record<string, unknown>, DataErasureRequestParams>, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    const renderModel: Pick<DataErasureRequestParams, 'email' | 'securityAnswer'> = {
    ...
    if (req.body.layout) {
      ...
      if (!isForbiddenFile) {
        res.render('dataErasureResult', renderModel, (error, html) => {
          if (!html || error) {
      ...
      res.render('dataErasureResult', renderModel)
    }
  } catch (error) {
    next(error)
  }
